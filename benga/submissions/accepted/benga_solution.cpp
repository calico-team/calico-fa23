...
int solve(string& S, Mat const& A) {
	...
	if (N == 0) return 0;
	N += 1;
	if (N > LAMBDA) return solveLargeN(N, A);
	else return solveSmallN(N.longValue(), A);
}
...
int main() {
	...
	// Mat x0 = makeMat(sz(A), 1);
	// x0[0][0] = 1;

	// cout << "MOD = " << MOD << '\n';
	// cout << "LAMBDA = " << floydCycleFinding(A) << '\n';
	...
	*/
	...
	// F0R(i, sz(pot2)) {
	// 	F0R(j, sz(pot3)) {
	// 		if (pot2[i] * pot3[j] < 679093956LL)
	// 			possible.insert(pot2[i] * pot3[j]);
	// 	}
	// }

	// each(p, possible) cout << p << endl;
	// cout << '\n';
